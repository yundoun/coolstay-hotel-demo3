...
export function StepReview() {
  const {
    ...
    selectedRoom, guestInfo, totalPrice,
    ...
    <div className="space-y-8">
      ...
      <div className="bg-white rounded-lg p-6 space-y-4 border border-neutral-200">
        ...
        <div className="grid grid-cols-2 gap-4 text-sm">
          <InfoItem label="호텔" value={siteConfig.name} />
      ...
      <div className="border border-neutral-200 rounded-lg p-6 space-y-3 bg-white">
        ...
        <div className="flex justify-between text-sm">
          ...
          <span className="text-neutral-800">₩{formatPrice(Math.round(totalPrice * 0.1))}</span>
        </div>
        <div className="border-t border-neutral-200 pt-3 flex justify-between">
          <span className="font-semibold text-neutral-900">총 결제 금액</span>
          <span className="text-xl font-bold text-neutral-900">₩{formatPrice(Math.round(totalPrice * 1.1))}</span>
        </div>
      </div>
      ...
      <div className="flex gap-3">
        ...
        <button onClick={handleComplete} className="flex-1 h-14 bg-neutral-900 hover:bg-neutral-800 text-white font-bold text-sm tracking-wide transition-colors">
          결제하기
        </button>
      </div>
    </div>
